...
public class App {
	private static final Comparator<Instituto> COMPARADORNOMBRESINSTITUTO = (p1,p2)-> p1.getNombre().compareTo(p2.getNombre());
	private static final Consumer<Object> ESCRIBIDOR = System.out::println; 
	private static final Predicate<Persona> MAYORDEEDAD = p -> p.getFechaNacimiento().plusYears(18).isBefore(LocalDate.now());
...
//		Listado del nombre y todos los datos de todos los teléfonos de contactos de todas las personas menores de edad por nombre de instituto.
		...
		institutos.stream().collect(Collectors.groupingBy(Instituto::getNombre,
				Collectors.flatMapping(i -> i.getPersonas().stream().filter(p->p.getFechaNacimiento().plusYears(18).isAfter(LocalDate.now())).map(p -> p.getNombre() + "->" + p.getTelefonos()), Collectors.toList()))).entrySet().forEach(ESCRIBIDOR);
